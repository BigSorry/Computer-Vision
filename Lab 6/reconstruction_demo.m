...
function [] = reconstruction_demo()
    ...
    addpath(genpath('D:\CS\IN4393-16 Computer Vision\Lab6'));
    addpath(genpath('C:\Users\Jos\Downloads\vlfeat-0.9.21-bin'));
    
    % Open the specified folder and read images. 
    directory = 'D:\CS\IN4393-16 Computer Vision\Lab6\TeddyBearPNG\' % Path to your local image directory 
    ...
    for iBegin = 1:n-(numFrames - 1)
        ...
        colInds = find(block(1,:) & block(2,:) & block(3,:));
        % colInds = find(all(block)==1);
        ...
        for f = 1:numFrames
            for p = 1:numPoints
                X(2 * f - 1, p) = C{1,iBegin+f-1}(1,block(f,p));
                X(2 * f, p)     = C{1,iBegin+f-1}(2,block(f,p));
            end
        end
    ...
    % q = 7;
    % plot3(Clouds{q,2}(1,:),Clouds{q,2}(2,:),Clouds{q,2}(3,:),'b.');
